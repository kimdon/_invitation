...
function setupZoomPrevention() {
  ...
  document.addEventListener("touchmove", (event) => {
    if (event.touches.length > 1 || multiTouch) {
      ...
      const touch = event.touches[0];
      if (touch && (Math.abs(touch.clientX - start.x) > 12 || Math.abs(touch.clientY - start.y) > 12)) start = lastTap = null;
    }
  }, options);
  document.addEventListener("touchend", (event) => {
    ...
    if (touch && Math.abs(touch.clientX - start.x) < 12 && Math.abs(touch.clientY - start.y) < 12) {
      const elapsed = lastTap ? event.timeStamp - lastTap.time : Infinity;
      if (elapsed >= 0 && elapsed < 300 && Math.abs(touch.clientX - lastTap.x) < 24 && Math.abs(touch.clientY - lastTap.y) < 24) {
        // CSS blocks zoom on controls; preserve the click generated by each tap.
        if (!event.target?.closest?.("button, a, input, select, textarea, [role='button']")) prevent(event);
        lastTap = null;
      } else {
  ...
  for (const type of ["gesturestart", "gesturechange", "gestureend"]) {
    document.addEventListener(type, (event) => { start = lastTap = null; prevent(event); }, options);
  }
  document.addEventListener("dblclick", prevent, options);
  document.addEventListener("wheel", (event) => { if (event.ctrlKey) prevent(event); }, options);
...
function setupPhotoViewer() {
  ...
  function prepareImage(number, priority = "high") {
    let entry = imageCache.get(number);
    if (!entry) {
      const image = document.createElement("img");
      image.alt = `사진 ${number}`;
      image.decoding = "async";
  ...
  function prepareNeighbors() {
    ...
    // Decode just the two neighbors, not the entire gallery. Failed prefetches
    // are silent and evicted so a later explicit navigation can retry them.
    for (const number of [nextNumber, previousNumber]) prepareImage(number, "low").catch(() => {});
  }
  ...
  async function render(number, direction = 0) {
    ...
    status.textContent = "";
    try {
      const image = await prepareImage(number);
